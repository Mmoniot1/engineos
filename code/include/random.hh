...
static PCG pcg_global = PCG_INITIALIZER;
...
static inline uint64_t pcg_random64(PCG* rng = &pcg_global);
...
static inline void pcg_seed(uint64_t initstate, uint64_t seq);
static inline void pcg_seed(PCG* rng, uint64_t seed);
static inline void pcg_seed(uint64_t seed);

uint64_t _pcg_advance_lcg_64(uint64_t state, uint64_t delta, uint64_t cur_mult, uint64_t cur_plus);

static inline void pcg_advance(PCG* rng, uint64_t delta);

// pcg_random_bound(bound):
// pcg_random_bound(rng, bound):
//     Generate a uniformly distributed number, r, where lower <= r <= upper
static inline uint32_t pcg_random32_in(PCG* rng, uint32_t lower, uint32_t upper);
static inline uint32_t pcg_random32_in(uint32_t lower, uint32_t upper);

// pcg_random_uniform(bound):
// pcg_random_uniform(rng, bound):
//     Generate a uniformly distributed number, r in [0, 1)
static inline float pcg_random_uniform(PCG* rng = &pcg_global);
//     Generate a uniformly distributed number, r in [0, 1]
static inline float pcg_random_uniform_in(PCG* rng = &pcg_global);
//     Generate a uniformly distributed number, r in (0, 1)
static inline float pcg_random_uniform_ex(PCG* rng = &pcg_global);
...
static inline float pcg_random_uniform(PCG* rng) {
	constexpr float NEG_32 = 1/(((float)~0u) + 1);
	uint32_t r = pcg_random32(rng);
	return r*NEG_32;
}
static inline float pcg_random_uniform_in(PCG* rng) {
	uint32_t r = pcg_random32(rng);
	return ((float)r)/((float)~0u);
}
static inline float pcg_random_uniform_ex(PCG* rng) {
	uint32_t r = pcg_random32(rng);
	return (((float)r) + 1)/(((float)~0u) + 2);
}

inline uint64_t pcg_random64(PCG* rng) {
